...
__version__ = '?'
exec(open(os.path.join("lib", "${PROJECT_MODULE}", "release.py")).read())
...
class build_ext(_build_ext):
  ...
  def finalize_options(self):
    ...
    if not isinstance(self.define, list):
      self.define = []
    self.define.append(('${PROJECT_MODULE_UPPER}_VERSION', '"%s"' % __version__))
  ...
  def _write_system_config_h(self):
    ...
    try:
      try:
        ...
        f.write("#ifndef ${PROJECT_MODULE_UPPER}_SYSTEM_CONFIG_H\n")
        f.write("#define ${PROJECT_MODULE_UPPER}_SYSTEM_CONFIG_H\n\n")
